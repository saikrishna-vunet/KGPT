...
class Beam(object):
    ...
    def __init__(self, size, pad_idx, sos_idx, eos_idx, device=False):
        ...
        self.next_ys = [torch.full(
            (size,), self.pad_idx, dtype=torch.long, device=device)]
    ...
    def advance(self, word_prob):
        ...
        for i in range(self.size):
            self.finished.append(
                self.next_ys[-1][i].item() in [self.eos_idx, self.pad_idx])
...
class KnowledgeEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.entity_embeddings = nn.Embedding(
            config.max_entity_embeddings, config.hidden_size)
        self.triple_embeddings = nn.Embedding(
            config.max_triple_embeddings, config.hidden_size)
    ...
    def forward(self, input_ids, entity_ids, triple_ids, position_ids):
        ...
        embeddings = inputs_embeds + entity_embeddings + \
            triple_embeddings + position_embeddings
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, config, n_head, n_layers):
        ...
        self.post_word_emb = PositionalEmbedding(
            d_model=config.hidden_size, max_len=1024)

        self.enc_layer_stack = nn.ModuleList([
            EncoderLayer(config.hidden_size, d_inner, n_head, d_k,
                         d_v, dropout=config.hidden_dropout_prob)
            for _ in range(n_layers)])

        self.dec_layer_stack = nn.ModuleList([
            DecoderLayer(config.hidden_size, d_inner, n_head, d_k,
                         d_v, dropout=config.hidden_dropout_prob)
            for _ in range(n_layers)])
    ...
    def last_layer(self, dec_output):
        if self.proj is not None:
            ...
            logits = torch.matmul(
                dec_output, self.embedding.word_embeddings.weight.transpose(1, 0))
        return logits

    def forward(self, input_ids, entity_ids, triple_ids, position_ids, tgt_seq):
        enc_inp = self.embedding(
            input_ids, entity_ids, triple_ids, position_ids)

        # -- Encode source
        non_pad_mask = get_non_pad_mask(input_ids, self.pad_idx)
        slf_attn_mask = get_attn_key_pad_mask(
            seq_k=input_ids, seq_q=input_ids, pad_idx=self.pad_idx)
        ...
        slf_attn_mask_keypad = get_attn_key_pad_mask(
            seq_k=tgt_seq, seq_q=tgt_seq, pad_idx=self.pad_idx)
        slf_attn_mask = (slf_attn_mask_keypad + slf_attn_mask_subseq).gt(0)
        dec_enc_attn_mask = get_attn_key_pad_mask(
            seq_k=input_ids, seq_q=tgt_seq, pad_idx=self.pad_idx)

        dec_output = self.embedding.word_embeddings(
            tgt_seq) + self.post_word_emb(tgt_seq)
    ...
    def greedy_decode(self, input_ids, entity_ids, triple_ids, position_ids, banwords=[], max_token_seq_len=30):
        ...
        tgt_seq = torch.LongTensor(batch_size, 1).fill_(
            self.sos_idx).to(input_ids.device)
    ...
    def beam_search(self, inputs, tokenizer, n_bm, max_token_seq_len=30, banwords=[]):
        ...
        with torch.no_grad():
            ...
            inputs = [_.repeat(1, n_bm).view(n_inst * n_bm, len_s)
                      for _ in inputs]

            # -- Prepare beams
            inst_dec_beams = [Beam(n_bm, tokenizer.pad_token_id, tokenizer.bos_token_id,
                                   tokenizer.eos_token_id, device=inputs[0].device) for _ in range(n_inst)]
            ...
            for len_dec_seq in range(1, max_token_seq_len + 1):
                ...
                inputs, inst_idx_to_position_map = collate_active_info(
                    inputs, inst_idx_to_position_map, active_inst_idx_list, n_bm)

            batch_hyp, batch_scores = collect_hypothesis_and_scores(
                tokenizer.pad_token_id, inst_dec_beams, n_bm)
...
class GatedTransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids, entity_ids, triple_ids, position_ids, tgt_seq,  positionwise_copy_prob=False):
        ...
        full_vocab_prob = full_vocab_prob.scatter_add(2, input_ids.unsqueeze(
            1).repeat(1, full_vocab_prob.shape[1], 1), oov_vocab_prob * copy_gate)
...
class GraphGatedTransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids, term_level_mask, triple_level_mask, entity_level_mask, fact_level_mask, gather_level_mask, tgt_seq, positionwise_copy_prob=False):
        enc_output = self.encode(input_ids, term_level_mask, triple_level_mask,
                                 entity_level_mask, fact_level_mask, gather_level_mask)
        # -- Prepare masks
        non_pad_mask = get_non_pad_mask(tgt_seq, self.pad_idx)
    ...
    def beam_search(self, inputs, tokenizer, n_bm, max_token_seq_len=30, banwords=[]):
        with torch.no_grad():
            ...
            for _ in inputs:
                if len(_.shape) == 2:
                    new_inputs.append(
                        _.repeat(1, n_bm).view(n_inst * n_bm, len_s))
                else:
                    new_inputs.append(_.repeat(1, n_bm, 1).view(
                        n_inst * n_bm, len_s, len_s))
            inputs = new_inputs
...
def collate_active_info(inputs, inst_idx_to_position_map, active_inst_idx_list, n_bm):
    ...
    active_inputs = [collect_active_part(
        _, active_inst_idx, n_prev_active_inst, n_bm) for _ in inputs]
...
def collect_hypothesis_and_scores(pad_idx, inst_dec_beams, n_best):
    ...
    for beam in inst_dec_beams:
        ...
        hyps = np.array([beam.get_hypothesis(i)
                        for i in range(beam.size)], 'long')
        lengths = (hyps != pad_idx).sum(-1)
        normed_scores = [scores[i].item()/lengths[i]
